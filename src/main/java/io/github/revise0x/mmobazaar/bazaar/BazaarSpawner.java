...
public class BazaarSpawner {
    ...
    public void spawn(Player owner, String bazaarName) {
        // TODO Disable spawning bazaars to close to other bazaars.
        Location baseLoc = owner.getLocation().getBlock().getLocation().add(Math.random() * 0.4 + 0.3, 0, Math.random() * 0.4 + 0.3);
        World world = baseLoc.getWorld();
        if (world == null) return;

        // 1. Armor Stand
        ArmorStand visual = world.spawn(baseLoc.clone(), ArmorStand.class);
        visual.setVisible(false);
        visual.setGravity(false);
        visual.setMarker(true);
        visual.setBasePlate(false);
        visual.setInvulnerable(true);
        Objects.requireNonNull(visual.getEquipment()).setHelmet(new ItemStack(Material.CHEST));
        visual.setHeadPose(new EulerAngle(0, 0, 0));

        // 2. Hologram 1 – Bazaar name
        ArmorStand nameLine = world.spawn(baseLoc.clone().add(0, 1.15, 0), ArmorStand.class);
        nameLine.setCustomName("§6" + bazaarName);
        ...
        ownerLine.setCustomName("§7by " + owner.getName());
